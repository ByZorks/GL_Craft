...
void Tree::generateVoxel() {
    ...
    for (int y = 5; y < 7; y++) {
        for (int x = 2; x < 5; x++) {
            for (int z = 2; z < 5; z++) {
                if ((x == 2 && z == 2) || (x == 4 && z == 4) || (x == 2 && z == 4) || (x == 4 && z == 2)) continue; // Skip corners
                m_blockType[index(x, y, z)] = BlockType::LEAVES;
            }
        }
    }
        ...
        for (int y = 0; y < SIZE; ++y) {
            for (int z = 0; z < SIZE; ++z) {
                ...
                if (shouldDrawFace(x, y + 1, z, blockType)) {
                    Block::addFaceVertices(Face::TOP, blockType, m_vertices, static_cast<float>(x), static_cast<float>(y), static_cast<float>(z));
                    m_blockFaceData.emplace_back(Face::TOP, 4);
                }
                if (shouldDrawFace(x, y - 1, z, blockType)) {
                    Block::addFaceVertices(Face::BOTTOM, blockType, m_vertices, static_cast<float>(x), static_cast<float>(y), static_cast<float>(z));
                    m_blockFaceData.emplace_back(Face::BOTTOM, 4);
                }
                if (shouldDrawFace(x, y, z + 1, blockType)) {
                    Block::addFaceVertices(Face::FRONT, blockType, m_vertices, static_cast<float>(x), static_cast<float>(y), static_cast<float>(z));
                    m_blockFaceData.emplace_back(Face::FRONT, 4);
                }
                if (shouldDrawFace(x, y, z - 1, blockType)) {
                    Block::addFaceVertices(Face::BACK, blockType, m_vertices, static_cast<float>(x), static_cast<float>(y), static_cast<float>(z));
                    m_blockFaceData.emplace_back(Face::BACK, 4);
                }
                if (shouldDrawFace(x + 1, y, z, blockType)) {
                    Block::addFaceVertices(Face::RIGHT, blockType, m_vertices, static_cast<float>(x), static_cast<float>(y), static_cast<float>(z));
                    m_blockFaceData.emplace_back(Face::RIGHT, 4);
                }
                if (shouldDrawFace(x - 1, y, z, blockType)) {
                    Block::addFaceVertices(Face::LEFT, blockType, m_vertices, static_cast<float>(x), static_cast<float>(y), static_cast<float>(z));
...
void Tree::setupBuffers() {
    ...
    MeshIndices.reserve(m_vertices.size() * 6); // 6 indices per face
    ...
    for (const auto& [faceType, vertexCount] : m_blockFaceData) {
        ...
        const unsigned int* indices = faceType == Face::BACK || faceType == Face::LEFT || faceType == Face::TOP
            ...
            : faceIndicesCCW;
        for (int i = 0; i < 6; ++i) {
            MeshIndices.push_back(vertexOffset + indices[i]);
        }
        vertexOffset += vertexCount;
    }

    m_VBO.init(m_vertices.data(), m_vertices.size() * sizeof(BlockVertex));
    m_IBO.init(MeshIndices.data(), MeshIndices.size());
