...
private:
    static BlockVertex packVertexData(const std::array<unsigned int, 3> &position, uint8_t texLayer, unsigned int faceIndex,
                                      const std::array<unsigned int, 4> &ao, uint8_t sunlight, const RGBLight& blockLight);
...
private:
    static constexpr std::array<std::array<uint8_t, 3>, 29> s_textureLayer = {
        {
            // [side, top, bottom]
            {0, 0, 0},    // AIR
            {0, 0, 0},    // BEDROCK
