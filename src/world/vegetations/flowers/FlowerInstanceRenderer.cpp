...
void FlowerInstanceRenderer::init() {
...
void FlowerInstanceRenderer::addInstance(const glm::vec3 &position) {
...
void FlowerInstanceRenderer::updateInstanceBuffer() {
...
void FlowerInstanceRenderer::resetInstances() {
    m_instanceCount = 0;
}

void FlowerInstanceRenderer::draw() const {
    if (m_instanceCount == 0 || !m_buffersInitialized) return;

    Renderer::drawInstanced(m_VAO, m_IBO, m_instanceCount);
}

unsigned int FlowerInstanceRenderer::m_instance_count() const {
